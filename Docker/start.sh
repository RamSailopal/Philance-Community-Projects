...
if [ $VM_EXISTS_CODE -eq 1 ]; then
	...
	"${VBOXMANAGE}" sharedfolder add "${VM}" -name "Philance" -hostpath "C:\Hashicorp\Philance\Share\Philance" --automount
	"${VBOXMANAGE}" sharedfolder add "${VM}" -name "Media" -hostpath "C:\Hashicorp\Philance\Share" --automount
	"${VBOXMANAGE}" setextradata "${VM}" VBoxInternal2/SharedFoldersEnableSymlinksCreate/Philance 1 
...
"${DOCKER_MACHINE}" ssh "${VM}" "sudo mkdir /c/Philance | sudo mount -t vboxsf Philance /c/Philance"
"${DOCKER_MACHINE}" ssh "${VM}" "sudo mkdir /c/Media | sudo mount -t vboxsf Media /c/Media"
...
read -p "Please take a note of the IP address above as you will need this address to navigate to the Philance platform on port 3000. Please enter to continue " resp
export -f docker
export hostaddy="$("${DOCKER_MACHINE}" ip ${VM})"
export repopath="/c/Philance"
export mediapath="/c/Media"
cd /c/Hashicorp/Philance/Share/Philance/Docker
docker-compose up
